...
public class SingleTouchView extends View {
    ...
    private static final int STATUS_ZOOM = 30;
    private static final int STATUS_DEL = 40;
     ...
     * 用于缩放，旋转的图标
     */
    private Drawable rtDrawable;//右上角
    private Drawable rbDrawable;//右下角
    private Drawable ltDrawable;//左上角
    private Drawable lbDrawable;//左下角
    ...
    private void obtainStyledAttributes(AttributeSet attrs) {
        ...
        rtDrawable = mTypedArray.getDrawable(R.styleable.SingleTouchView_rtDrawable);
        rbDrawable = mTypedArray.getDrawable(R.styleable.SingleTouchView_rbDrawable);
        ltDrawable = mTypedArray.getDrawable(R.styleable.SingleTouchView_ltDrawable);
        lbDrawable = mTypedArray.getDrawable(R.styleable.SingleTouchView_lbDrawable);
    ...
    private void init() {
        ...
        mPaint.setPathEffect ( new DashPathEffect( new float [ ] { 34,  17}, 0 ) ) ;
        if (rtDrawable == null) {
            rtDrawable = getContext().getResources().getDrawable(R.mipmap.home_btn_close, null);
        }

        if (rbDrawable == null) {
            rbDrawable = getContext().getResources().getDrawable(R.mipmap.home_btn_close, null);
        }

        if (ltDrawable == null) {
            ltDrawable = getContext().getResources().getDrawable(R.mipmap.home_btn_close, null);
        }

        if (lbDrawable == null) {
            lbDrawable = getContext().getResources().getDrawable(R.mipmap.home_btn_close, null);
        }

        mDrawableWidth = rtDrawable.getIntrinsicWidth();
        mDrawableHeight = rtDrawable.getIntrinsicHeight();
    ...
    private int JudgeStatus(float x, float y) {
        ...
        PointF rbPointF = new PointF(mRBPoint);

        PointF rtPointF = new PointF(mRTPoint);
        PointF ltPointF = new PointF(mLTPoint);
        PointF lbPointF = new PointF(mLBPoint);

        //点击的点到控制旋转，缩放点的距离
        float distanceToRb = distance4PointF(touchPoint, rbPointF);
        float distanceToRt = distance4PointF(touchPoint, rtPointF);
//        float distanceToLT = distance4PointF(touchPoint, ltPointF);
        float distanceToLb = distance4PointF(touchPoint, lbPointF);

        //如果两者之间的距离小于 控制图标的宽度，高度的最小值，则认为点中了控制图标
        if (distanceToRb < Math.min(mDrawableWidth / 2, mDrawableHeight / 2)) {
            return STATUS_ROTATE;
        } else if (distanceToLb < Math.min(mDrawableWidth / 2, mDrawableHeight / 2)) {
            return STATUS_ZOOM;
        } else if (distanceToRt < Math.min(mDrawableWidth / 2, mDrawableHeight / 2)) {
            //删除事件
            return STATUS_DEL;
        }
        return STATUS_DRAG;
